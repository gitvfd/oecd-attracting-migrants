function createChart(dimensionName) {
	...
	if (width > 800) {
		...
		lollipopsLine = svg.selectAll("circleLine")
			.data(data.filter(function (d) { return d.Dimension == dimensionName; }))
			.enter().append("line")
		...
		///Lollipop dimDesc
		lollipopsCircle = svg.selectAll("lollipopCircle")
			.data(data.filter(function (d) { return d.Dimension == dimensionName }))
			.enter().append("circle")
		...
		lollipopsText = svg.selectAll("lollipopText")
			.data(data.filter(function (d) { return d.Dimension == dimensionName && d.Cat=="Entrepreneurs"; }))
			.enter().append("text")
...
function update(dimensionName) {
	...
	if (width > 800) {
		...
		xScale = d3.scaleBand()
			.domain(data.filter(function (d) { return (d.Dimension == selDimPlaceholder); }).sort(function (a, b) { return parseFloat(a.Countries) - parseFloat(b.Countries); }).map(function (d) { return d.Countries }))
			.range([margin, width - margin / 2])
			.padding(padding);

		lollipopsCircle.data(data.filter(function (d) { return d.Dimension == dimensionName }))
			// .enter()
			.transition()
			.duration(1000)
			.ease(d3.easeBounce)
...
function sortupdate(topicSelected) {
	...
	if (width > 800) {
		...
		xScale = d3.scaleBand()
			.domain(data.filter(function (d) { return (d.Cat == sortFilter && d.Dimension == selDimPlaceholder); }).sort(function (a, b) { return parseFloat(b.value) - parseFloat(a.value); }).map(function (d) { return d.Countries }))
			.range([margin, width - margin / 2])
			.padding(padding);

		lollipopsCircle.data(data.filter(function (d) { return d.Dimension == selDimPlaceholder }))
			// .enter()
			.transition()
			.duration(1000)
			.ease(d3.easeBounce)
		...
		lollipopsText
			.data(data.filter(function (d) { return d.Dimension == selDimPlaceholder }))
			.transition()
			.duration(1000)
			.ease(d3.easeBounce)
			.attr("dy", function (d) {
